...
- Bilibili 音频 URL 约10分钟过期（`AUDIO_URL_EXPIRY_MS`，`bilibiliApi.ts:3`）。`useAudioPlayer.ts` 播放时每60秒刷新；`api.ts` 的 `loadAudioTrack` 在当前 URL 距过期不足60秒时跳过重新获取；`resumeAudioLocal` 恢复播放前也会检查 URL 是否过期并自动刷新
...
- F12 在开发模式下打开 DevTools（`windowManager.ts:66`，通过 `before-input-event` 注册，而非 `globalShortcut`）
- `api.ts:playAudioLocal` 使用 `canplaythrough` 而非 `canplay` — 蓝牙耳机场景下避免"进度条先走、声音后到"的问题
